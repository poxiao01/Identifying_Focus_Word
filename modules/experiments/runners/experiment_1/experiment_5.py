...
class Experiment(BaseExperiment):
    ...
    def save_results_to_excel(self):
        ...
        # 在保存路径文件名后添加时间戳
        file_name_with_timestamp = f"{os.path.splitext(self.save_file_path)[0]}_{timestamp}.xlsx"

        # 将数据保存到文件
        df = pd.DataFrame(formatted_results)
        df.to_excel(file_name_with_timestamp, index=False)

        logging.info(f"实验结果已保存到 {file_name_with_timestamp} 文件。")
...
def main():
    ...
    for train_file in train_files:
        ...
        config = {
            ...
            'min_confidences': [i / 100 for i in range(5, 31, 5)],  # 0.05 - 0.30
            'support_values': list(range(5, 31, 5))  # 5 - 30，每隔5一个
        }
